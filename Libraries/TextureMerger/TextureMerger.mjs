...
var TextureMerger = function(texturesObj){
  ...
  this.ranges = this.makeCanvasPowerOfTwo(undefined, this.ranges);
...
TextureMerger.prototype.makeCanvasPowerOfTwo = function(canvas, Ranges){
  //return Ranges;
  var setCanvas = false;
  if (!canvas){
    canvas = this.canvas;
  ...
  var newWidth = Math.pow(2, Math.ceil(Math.log(oldWidth) / Math.log(2)));
  var newHeight = Math.pow(2, Math.ceil(Math.log(oldHeight) / Math.log(2)));
  var newCanvas = document.createElement("canvas");
  newCanvas.width = newWidth;
  newCanvas.height = newHeight;
  newCanvas.getContext("2d").drawImage(canvas, 0, 0/*, newWidth, newHeight*/); //Didn't stretch out canvas so that textures remain sharp.
  if (setCanvas){
    this.canvas = newCanvas;
  }
  
  for(const Key in Ranges){
	Ranges[Key].startU *= oldWidth / newWidth;
	Ranges[Key].endU *= oldWidth / newWidth;
	Ranges[Key].startV = 1 - (1 - Ranges[Key].startV) * (oldHeight / newHeight);
	Ranges[Key].endV = 1 - (1 - Ranges[Key].endV) * (oldHeight / newHeight);
  }
  return Ranges;
}
